...
class FakeQuestionGenerationService : QuestionGenerationService {
    ...
    override suspend fun generateQuestions(tasks: List<Task>, questionCount: QuestionCount): List<String> {
        capturedTasks = tasks
...
class GenerateQuestionsServiceTest : BehaviorSpec({
    ...
    Given("질문 생성 요청이 왔을 때") {
        ...
        val questions = generatedQuestionContents.mapIndexed { index, content ->
            Question(
    ...
    Given("민감도가 NEVER인 할일이 포함되어 있을 때") {
        ...
        val completedRetrospective = existingRetrospective.copy(
            status = RetrospectiveStatus.AFTER_GENERATE_QUESTION,
            questions = questions
        )
