...
class GenerateQuestionsService(
    ...
    override suspend fun execute(command: RetrospectiveApplicationCommand.GenerateQuestions): RetrospectiveDto {
        // 1. 회고 조회
        val retrospective = retrospectiveRepository.findAll(
        ...
        // 5. AI 질문 생성
        val generatedQuestions = questionGenerationService.generateQuestions(
            tasks = filteredTasks,
            questionCount = retrospective.questionCount
        )

        // 6. 질문 생성 완료
        val savedRetrospectives = retrospectiveRepository.saveAll(listOf(
